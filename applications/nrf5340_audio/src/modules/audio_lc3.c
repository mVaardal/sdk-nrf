...
static uint16_t m_i2s_tx_buf_a[I2S_16BIT_SAMPLE_NUM], m_i2s_rx_buf_a[I2S_16BIT_SAMPLE_NUM],
	m_i2s_tx_buf_b[I2S_16BIT_SAMPLE_NUM], m_i2s_rx_buf_b[I2S_16BIT_SAMPLE_NUM];
...
void audio_lc3_i2s_callback(uint32_t frame_start_ts, uint32_t *rx_buf_released,
...
static int cmd_play_wav_file(const struct shell *shell, size_t argc, char **argv)
{
	...
	int ret = audio_lc3_play_file_from_sd(filename, sd_card_file_path);
	if (ret < 0) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	audio_lc3_cmd,
	SHELL_COND_CMD(CONFIG_SHELL, play_file, NULL, "Play file from SD card.", cmd_play_wav_file),
...
SHELL_CMD_REGISTER(audio_lc3, &audio_lc3_cmd, "Play .wav-files", NULL);
